...
class Board(object):
    ...
    def __hash__(self):
        return hash(str(hash(tuple(self.hall))) + str(hash(tuple(self.rooms))))

    def clone(self):
        return Board(self.hall[:], copy.deepcopy(self.rooms))

    def has_obstacle(self, from_idx, to_idx):
        return any(self.hall[i] != '.' for i in range(from_idx, to_idx))
    ...
    def pop_to_hall(self, room_idx, to_idx):
        ...
        if self.has_obstacle(room_idx+1, to_idx+1):
            return False
    ...
    def push_from_hall(self, from_idx, room_idx):
        ...
        if self.has_obstacle(from_idx+1, room_idx+1):
            return False
    ...
    def room_to_room(self, from_idx, to_idx):
        ...
        assert len(self.rooms[from_idx]) > 0
        assert len(self.rooms[to_idx]) < 2
...
def silver(board):
    ...
    def rec(cloned_board):

        # pop from room to hall
        for room_idx in Board.room_indices.values():
            board = cloned_board.clone()
            for to_idx in range(1, 13):
                if board.pop_to_hall(room_idx, to_idx) and hash(board) not in visited:
                    visited.add(hash(board))
                    rec(board)
                    board = cloned_board.clone()

        # move from hall to room
        for from_idx in range(1, 13):
            for room_idx in Board.room_indices.values():
                if board.push_from_hall(from_idx, room_idx) and hash(board) not in visited:
                    visited.add(hash(board))
                    rec(board)
                    board = cloned_board.clone()
...
def parse(lines):
    ...
    rooms = [[s] for s in lines[2].strip() if s != '#']
    rooms = [[s] + rooms[i//2]
