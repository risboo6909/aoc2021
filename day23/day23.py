...
class Board(object):
    ...
    def __str__(self):
        ...
        res += (
            "".join(
                (
                    get(self.rooms[i], self.room_size - 1) if i in self.rooms else "#"
                    for i in range(13)
                )
            )
            + "\n"
        ...
        for k in range(self.room_size - 2, -1, -1):
            res += (
                ...
                + "".join(
                    (
                        get(self.rooms[i], k) if i in self.rooms else "#"
                        for i in range(3, 10)
                    )
                )
                + "#  \n"
    ...
    def __hash__(self):
        return hash(
            (tuple(self.hall), tuple((k, tuple(v)) for k, v in self.rooms.items()))
        )
    ...
    def clone(self):
        return Board(
            self.hall[:], {k: v[:] for k, v in self.rooms.items()}, self.room_size
        )
    ...
    def pop_to_hall(self, room_idx, to_idx):
        ...
        return energy[item] * (
            abs(room_idx - to_idx) + self.room_size - len(self.rooms[room_idx])
        )
    ...
    def push_from_hall(self, from_idx, to_room_idx):
        ...
        return energy[item] * (
            abs(from_idx - to_room_idx)
            + self.room_size
            - len(self.rooms[to_room_idx])
            + 1
        )
...
def silver(board):
    return "silver", solver(board, 16)


def gold(board):
    return "gold", solver(board, 32)
...
def parse(lines, room_size):
    ...
    for k in range(room_size - 1):
        rooms = [
            [s] + rooms[i // 2]
            for i, s in enumerate(lines[3 + k].strip())
            if (i % 2) != 0
        ]
...
def solve():
    ...
    lines_silver = open(
        os.path.join(os.path.dirname(__file__), "input_silver"), "rt"
    ).readlines()
    board_silver = parse(lines_silver, room_size=2)

    lines_gold = open(
        os.path.join(os.path.dirname(__file__), "input_gold"), "rt"
    ).readlines()
    ...
    with Pool(processes=2) as p:
        res = dict(
            [
                worker.get()
                for worker in [
                    p.apply_async(silver, (board_silver,)),
                    p.apply_async(gold, (board_gold,)),
                ]
            ]
        )

    return "DAY23", res["silver"], res["gold"]
